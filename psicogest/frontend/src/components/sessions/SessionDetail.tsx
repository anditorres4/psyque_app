...
export function SessionDetail({ sessionId, onBack }: Props) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Invoice creation — only for signed sessions */}
      {isSigned && (
        <div className="rounded-lg border border-slate-200 p-4 space-y-2">
          <p className="text-sm font-medium text-[#1E3A5F]">Facturación</p>
          {invoiceCreated ? (
            <p className="text-sm text-green-700">Factura creada correctamente. Puedes verla en el módulo de Facturas.</p>
          ) : (
            <>
              {invoiceError && <p className="text-xs text-[#E74C3C]">{invoiceError}</p>}
              <Button
                size="sm"
                variant="outline"
