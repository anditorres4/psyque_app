...
try:
    ...
    _BOGOTA_TZ: object = ZoneInfo("America/Bogota")
except Exception:
...
class AppointmentService:
    ...
    def _to_bogota(self, dt: datetime) -> datetime:
        """Convert a datetime to Bogotá local time for availability comparison."""
        if dt.tzinfo is None:
            dt = dt.replace(tzinfo=timezone.utc)
        return dt.astimezone(_BOGOTA_TZ)  # type: ignore[arg-type]

    def _check_availability(self, start: datetime, end: datetime) -> None:
        """Raise AppointmentConflictError if the slot isn't covered by an active block.

        Skipped when the tenant has no configured blocks (new account grace period).
        """
        ...
        if not has_any:
            return  # no blocks configured — allow all slots

        local_start = self._to_bogota(start)
        local_end = self._to_bogota(end)
        day = local_start.weekday()  # Monday=0, Sunday=6
        ...
        block = (
            ...
            .filter(
                AvailabilityBlock.tenant_id == tenant_uuid,
                AvailabilityBlock.is_active.is_(True),
                AvailabilityBlock.day_of_week == day,
                AvailabilityBlock.start_time <= t_start,
                AvailabilityBlock.end_time >= t_end,
            )
            .first()
        )
        if not block:
            raise AppointmentConflictError(
                "No hay disponibilidad configurada para ese horario. "
                "Revisa los bloques de disponibilidad en Configuración."
            )
    ...
    def create(self, data: dict) -> Appointment:
        """Create appointment. Raises AppointmentConflictError on overlap or outside availability."""
        self._check_availability(data["scheduled_start"], data["scheduled_end"])
    ...
    def update(self, appointment_id: str, data: dict) -> Appointment:
        ...
        if "scheduled_start" in data or "scheduled_end" in data:
            self._check_availability(new_start, new_end)
